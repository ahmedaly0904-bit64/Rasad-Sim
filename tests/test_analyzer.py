...
def test_the_mean_interval_covers_the_true_mean_about_90_percent_of_the_time(n):
    """The label says 90%; at the small run counts this library allows it
    must mean it. The bootstrap it replaces measured 50%, 79% and 84% here."""
    rng = np.random.default_rng(3)
    trials = 2000
    covered = 0
    for _ in range(trials):
        result = summarize(rng.normal(10.0, 2.0, n).tolist())
        covered += result["ci_low"] <= 10.0 <= result["ci_high"]
    assert covered / trials == pytest.approx(0.90, abs=0.02)


def test_a_constant_sample_has_a_zero_width_mean_interval():
    result = summarize([4.0, 4.0, 4.0])
    assert result["ci_low"] == result["ci_high"] == 4.0
